...
/**
 ...
 `sceneXML` is stored as `scene.xml`. Resource keys are flat archive filenames and
 values are local file URLs. Resource filenames ending in `.xml` are rejected so
 that every successful archive contains exactly one XML entry.

 The returned metrics contain numeric values for `entry_count`, `xml_count`,
 `uncompressed_bytes`, `compressed_bytes`, `archive_bytes`, `required_bytes`,
 `free_bytes_before`, and `crc_verified`.
 */
