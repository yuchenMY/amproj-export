...
- iOS 原生 PackageImporter 若直接产生底部项目，插件会连续确认模板列表与导入前基线完全一致，并在完成事务前再做一次最终复查。
- 若原生流程只产生模板，该模板仅作为中间转换状态；插件随后调用 AM 官方模板选择流程创建项目，并按 AM 官方菜单清理本次临时模板。
...
因此，`2/4` 或 `3/4` 停住、闪退的旧 v20-v30 包不要继续重复安装；请使用 v33 构建。v33 会在私有工作副本中补齐 iOS 所需的媒体 SHA-1，再把 XML 与全部图片、音频、视频和字体作为一个完整项目包交给 AM，随后自动完成“模板 → 项目”的官方转换。
...
复制完成后的处理全部在本地执行：XML 只做结构校验并送入模板文档入口；`.amproj` 逐项解压验证 ZIP32、local header、CRC、XML、manifest SHA-1 和路径安全，再核对所有 `amproj:` 素材引用。缺少任一图片、音频、视频或字体时，会在进入原生 `PackageImporter` 前停止并保留缓存包；资源齐全但缺 iOS 媒体 `sig` 时，重建包含全部资源的兼容工作包。若原生导入直接产生项目，插件会确认没有新增临时模板；若产生模板，则通过标准 collection/table delegate 选择本次新增模板、创建项目并安全清理它。
...
下载名为 `AMProjExport-v33-dylibs` 的 artifact。其中的 `build-metadata.json`
...
python .\inject_dylib.py .\AM_v1.ipa .\AMProjExport\AMProjExport.dylib .\AM_v1_direct_v33.ipa `
...
python .\inject_dylib.py .\AM_v1.ipa .\AMProjExport\AMProjExportCloud.dylib .\AM_v1_direct_v33_cloud.ipa `
  --server-url https://bug.meowcr.cn --no-discovery --debug-mode full `
  --debug-token $token --build-id v33-cloud-<commit> --expected-main-uuid $uuid
```
...
python .\inject_dylib.py .\AM_v1.ipa .\AMProjExport\AMProjExportDebug.dylib .\AM_v1_direct_v33_debug.ipa `
...
$uuid = "4b22d43f-09fc-3bde-859b-78a5d573a503"
$token = "<与云端后端一致的 Bearer token>"
python .\inject_dylib.py .\AM_v1.ipa .\AMProjExport\AMProjExportDebug.dylib .\AM_v1_direct_v33_cloud_debug.ipa `
  --server-url https://bug.meowcr.cn --no-discovery --debug-mode full `
  --debug-token $token --build-id v33-debug-<commit> --expected-main-uuid $uuid
```
...
$uuid = "4b22d43f-09fc-3bde-859b-78a5d573a503"
$token = python -c "import secrets; print(secrets.token_urlsafe(32))"
python .\inject_dylib.py .\AM_v1.ipa .\AMProjExport\AMProjExportDebug.dylib .\AM_v1_direct_v33_share_exp.ipa `
  --debug-mode full --debug-token $token --expected-main-uuid $uuid `
